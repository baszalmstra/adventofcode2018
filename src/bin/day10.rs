...
fn main() {
    let regex = regex::Regex::new(r"position=<\s*(-?\d+),\s*(-?\d+)> velocity=<\s*(-?\d+),\s*(-?\d+)>").unwrap();
    let input:Vec<(Point,Point)> = std::fs::read_to_string("inputs/day10/input")
        .expect("Could not read input file")
        .lines()
        .map(|l| {
            let captures = regex.captures(l).unwrap();
            (Point::new(captures[1].parse().unwrap(),
                        captures[2].parse().unwrap()),
             Point::new(captures[3].parse().unwrap(),
    ...
    for y in 0..height {
        for x in 0..width {
            print!("{}", if grid[(y*width+x) as usize] { "#" } else { " " } );
        }
        println!();
    }
